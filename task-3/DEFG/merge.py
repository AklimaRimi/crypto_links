...
# file1 = pd.read_csv('updatedG7 .csv')
file1 = pd.read_csv('G7 - G7.csv')
...
file1.to_csv('updatedFilterEmailG7.csv',index = False)
for i in  file2['Email'].values.tolist():
    ...
    for w in range(len(file1['web'].values.tolist())):
        # print(file1['web'][w],i)
        comp = file1['web'][w]
        if len(comp) > 4 and ((i1 ==  comp) or (comp == i1) or (comp.find(i1) != -1) or (i1.find(comp) != -1)):
...
file1.to_csv('updatedG7 .csv',index = False)
